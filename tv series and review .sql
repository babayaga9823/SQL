...
select title,
genre,
ifnull(avg(rating),'not rated yet') as 'overall rating'
from series left join reviews on 
...
--each reviewer analysis
select concat(reviewers.first_name,' ',reviewers.last_name) as name,
count(reviewers.id) as 'Total reviews',
min(rating) as 'min',
max(rating) as 'max',
avg(rating) as 'Avg rating',
case 
when avg(rating)>5 then 'active'
else 'not active'
end as status
from reviewers
left join reviews
